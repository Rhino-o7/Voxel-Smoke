...
namespace yc {
    ...
    void Application::process() {
        ...
        const int desiredCursorMode = (!windowFocused || saveSelectionActive || paused || gameManager.isGameOver())
            ? GLFW_CURSOR_NORMAL
            : GLFW_CURSOR_DISABLED;

        static int appliedCursorMode = GLFW_CURSOR_NORMAL;
        if (desiredCursorMode != appliedCursorMode) {
            glfwSetInputMode(window, GLFW_CURSOR, desiredCursorMode);
            appliedCursorMode = desiredCursorMode;
        }
        ...
        if (!saveSelectionActive && !paused && !gameManager.isGameOver()) {
            const int actualCursorMode = glfwGetInputMode(window, GLFW_CURSOR);
            if (actualCursorMode != GLFW_CURSOR_DISABLED) {
                pauseGame();
            }
        }
    ...
    void Application::saveCurrentGame() {
        if (saveSystem.getCurrentSaveName().empty()) {
            return;
        }
