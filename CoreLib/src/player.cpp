...
void Player::moveFront() {
    auto delta = speed * yc::runtime_state::GetDeltaTime() * camera.getFront();
    if (!flyMode) {
        delta.y = 0.0f;
...
void Player::moveRight() {
    auto delta = speed * yc::runtime_state::GetDeltaTime() * camera.getRight();
    if (!flyMode) {
        delta.y = 0.0f;
        moveWithCollision(delta);
        return;
...
void Player::moveUp() {
    ...
    auto delta = speed * yc::runtime_state::GetDeltaTime() * VectorUp;
    if (checkIntersect(delta)) return;
    camera.setPosition(camera.getPosition() + delta);
}
